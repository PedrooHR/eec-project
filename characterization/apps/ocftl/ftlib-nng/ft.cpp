  ...
  for (int i = 0; i < size; i++) {
    ...
    if (i != rank) { // No self Sockets
      ...
      if (rank < i) {
        ...
        int node = (rank / ((int) std::ceil(size/3.0))) + 1;
        //int node = (rank / ((int) std::ceil(size/(2.0)))) + 2;
	// FTDEBUG("[Rank %d] - Im in node %d\n", rank, node);
        urls[i] =
            "ws://sorgan-gpu" + std::to_string(node) + ".cluster:" + std::to_string(port) + "/" + std::to_string(i);
        FTDEBUG("[Rank %d] Trying to lister for %d (%s)\n", rank, i,
                  urls[i].c_str());
        do {
          result = nng_listen(sockets[i], urls[i].c_str(), NULL, 0);
          std::this_thread::sleep_for(std::chrono::milliseconds(1));
        } while (result != 0);
        FTDEBUG("[Rank %d] Listening for %d (%s)\n", rank, i,
                  urls[i].c_str());

      } else {
        int result;
        int port = base_port + i;
        int node = (i / ((int) std::ceil(size/3.0))) + 1;
        //int node = (i / ((int) std::ceil(size/(2.0)))) + 2;
        urls[i] = "ws://sorgan-gpu" + std::to_string(node) + ".cluster:" + std::to_string(port) + "/" +
                  std::to_string(rank);
        FTDEBUG("[Rank %d] Trying to dial %d (%s)\n", rank, i,
                  urls[i].c_str());
        do {
          result = nng_dial(sockets[i], urls[i].c_str(), NULL, 0);
          std::this_thread::sleep_for(std::chrono::milliseconds(1));
        } while (result != 0);
        FTDEBUG("[Rank %d] Dialed %d (%s)\n", rank, i,
                  urls[i].c_str());
      }
    } else {
  ...
  while (!hb_done) {
    ...
    for (int sender = 0; sender < app_size; sender++) {
      if (open_sockets[sender]) {
        ...
        if (result != 0 && result != NNG_EAGAIN) {
          FTDEBUG("[Rank %d] - Error %d receiving from sender %d\n", rank, result, sender);
        }
        if (result == 0) {
...
void FaultTolerance::hbFindDeadNode() {
  ...
  printf("[Rank %d FT] Found a failed process: %d\n", rank, emitter);
